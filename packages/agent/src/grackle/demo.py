...
def _seed_session_store(fixture_roots: dict[str, Path]) -> SessionStore:
...
class _DemoServer:
    ...
    async def _replay_loop(self, ws: ServerConnection, root: Path) -> None:
        """Replay root/trace.golden.jsonl to ws; repeat if --loop was given.
        ...
        trace_path = root / "trace.golden.jsonl"
        try:
            while True:
...
async def serve_demo(
    ...
    store = _seed_session_store(fixture_roots)
    try:
        server = _DemoServer(
            fixture_roots, default=default, store=store, loop_trace=loop_trace, pace=pace
        )
