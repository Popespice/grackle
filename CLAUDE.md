...
**Phase 8 candidates:** tee sink (`--stream + --output`); server-side prefix aggregates for cumulative heat over seek windows; polyglot static depth (gRPC/protobuf cross-language edges, generics-aware Rust resolver, re-export chasing); sparse `JsonlIndex`; agent-side cycle detection.
