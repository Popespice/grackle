...
- **POSIX path discipline.** All path-bearing fields that cross the wire or persist (node IDs, annotation keys, manifest entries) must be POSIX-relative strings — `services/auth.py`, never `services\auth.py`. Use `grackle.paths.to_posix(p, root)`; do not call `.relative_to()` directly outside `paths.py`. A single missed call site silently diverges IDs between macOS and Windows. The `ruff PTH` ruleset guards the general case. A dedicated path-discipline lint test does **not** exist yet (planned as campaign T3-4, `docs/test-campaigns/phase-12.md`); until it lands the invariant is convention-only, and `cli.py:437` and `python_runtime/node_resolution.py:91` both call `.relative_to()` directly — arguably legitimate containment predicates, but that distinction is currently written down nowhere.
