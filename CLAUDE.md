...
**Phase 8 (analysis platform) is in progress.** Chunk numbering follows the source plan (`~/.claude/plans/outline-phase-8-analysis-platform.md`), anchored on ADRs 0018–0022. **Shipped:** **8.2** — call-tree reconstruction + flame graph (PR #31, ADR-0019); **8.3** — server-side aggregation engine + trace persistence + session library (PR #32, ADR-0018 + ADR-0020); **8.4** — differential analysis (PR #33, ADR-0021): `diff.py` (trace-vs-static + trace-vs-trace) + `grackle diff A.jsonl B.jsonl` CLI (exit 1 on regression) + `DiffPanel` + opt-in graph overlay; and the **tee sink** (`--stream + --output`, PR #30) — a down-payment on 8.3's `MultiSink` (ADR-0020), pulled forward and shipped standalone. (Its commit/PR/tag are labeled "8.1"; that informal label is **retired** here because it collides with the plan's actual 8.1, the aggregation engine.) **Remaining:** **8.5** polyglot Node/V8 runtime (**ADR-0022 — proposed, design locked**); **8.6** tech-debt sweep; **8.H** ship (v0.8.0, PHASE_8_SUMMARY, PROJECT_ACCEPTANCE §D, tag v0.8.0-phase-8). Net: **3 chunks remain** (8.5, 8.6, 8.H).

**Phase 8.5 design is locked (ADR-0022, proposed — not yet built).** `NodeRuntimeAdapter` (language `"typescript"`) drives Node over the V8 Inspector (CDP) on `127.0.0.1`, emits the same `TraceEvent` schema resolved to TS static-graph node IDs, and reuses the whole Phase 6–8 transport/UI. Key decisions: **hybrid two-channel mechanism** — CPU sampling profiler (faithful flame, post-run, via `trace()` → `--connect` replay / `-o`) **+** precise-coverage polling (live exact heat, via `trace_streaming()` → `--stream`); each maps to an existing transport mode so there's no double-count and no frontend change. Node-ID resolution relies on **type-stripping** (Node ≥ 22.6 `--experimental-strip-types` preserves line numbers + keeps `.ts` URLs) → `(url, line+1) → node_id` via a new TS `NodeResolver` mirroring `python_runtime/node_resolution.py`; pseudo-frames filtered; name/file fallbacks. Lifecycle: spawn `node --inspect-brk=127.0.0.1:0` + a shipped `bootstrap.mjs` done-signal; minimal CDP client over the existing `websockets` dep (**no new deps**). Capability-gated on `shutil.which("node")` + version (clean `ClickException`, never a crash). **CLI `trace` is currently hardcoded to Python** — 8.5 adds adapter-by-language dispatch. Build order: (a) CLI dispatch + gate + registration → (b) pure `profile_reconstruct` + `coverage_poll` + TS resolver (fixture-driven, no Node) → (c) CDP client + launcher lifecycle (riskiest spike) → (d) wire `trace()`/`trace_streaming()` + Node-gated e2e → (e) ADR accept + docs + gate. Out of scope → Phase 9: sourcemaps/compiled-`.js`, `.tsx` loaders, worker threads, Python↔Node cross-process correlation. Full plan in the memory file `project_phase_8_progress.md` + ADR-0022.
