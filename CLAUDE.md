...
**Phase 10 (planned) — north star: live growing graph + explanation layer.** Point grackle at any filesystem and watch the graph grow live as the system is built, with an explanation layer for how/why files connect and fire. First concrete step: **value capture → time-travel debugger** — capture argument/return values alongside the existing call-graph trace so execution can be scrubbed and inspected after the fact, not just visualized as frequency/coverage. No ADRs pre-committed; scope chunks once design work starts.
