...
**Phase 8 (analysis platform) is in progress.** Chunk numbering follows the source plan (`~/.claude/plans/outline-phase-8-analysis-platform.md`), anchored on ADRs 0018–0022. **Shipped:** **8.2** — call-tree reconstruction + flame graph (PR #31, ADR-0019); **8.3** — server-side aggregation engine + trace persistence + session library (PR #32, ADR-0018 + ADR-0020); and the **tee sink** (`--stream + --output`, PR #30) — a down-payment on 8.3's `MultiSink` (ADR-0020), pulled forward and shipped standalone. (Its commit/PR/tag are labeled "8.1"; that informal label is **retired** here because it collides with the plan's actual 8.1, the aggregation engine.) **In progress: 8.4** differential analysis (ADR-0021) — `diff.py` + `grackle diff` CLI + `DiffPanel` + graph overlay. **Remaining:** **8.5** polyglot Node/V8 runtime (ADR-0022); **8.6** tech-debt sweep; **8.H** ship (v0.8.0, PHASE_8_SUMMARY, PROJECT_ACCEPTANCE §D, tag v0.8.0-phase-8). Net: **3 chunks remain** after 8.4 lands (8.5, 8.6, 8.H).
