...
**Phase 8 (analysis platform) is in progress.** Chunk numbering follows the source plan (`~/.claude/plans/outline-phase-8-analysis-platform.md`), anchored on ADRs 0018–0022. **Shipped:** **8.2** — call-tree reconstruction + flame graph (PR #31, ADR-0019); **8.3** — server-side aggregation engine + trace persistence + session library (PR #32, ADR-0018 + ADR-0020); **8.4** — differential analysis (PR #33, ADR-0021): `diff.py` (trace-vs-static + trace-vs-trace) + `grackle diff A.jsonl B.jsonl` CLI (exit 1 on regression) + `DiffPanel` + opt-in graph overlay; **8.5** — polyglot Node/V8 runtime (**this PR**, ADR-0022 accepted): `node_runtime/` adapter (language `"typescript"`) drives Node over the V8 Inspector (CDP), emits the same `TraceEvent` schema, reuses the whole Phase 6–8 pipeline; and the **tee sink** (`--stream + --output`, PR #30) — a down-payment on 8.3's `MultiSink` (ADR-0020), pulled forward and shipped standalone. (Its commit/PR/tag are labeled "8.1"; that informal label is **retired** here because it collides with the plan's actual 8.1, the aggregation engine.) **Remaining:** **8.6** tech-debt sweep; **8.H** ship (v0.8.0, PHASE_8_SUMMARY, PROJECT_ACCEPTANCE §D, tag v0.8.0-phase-8). Net: **2 chunks remain** (8.6, 8.H). *(8.5's "shipped at main=<sha>" line is folded into the 8.6 PR once merged, per the per-chunk cadence.)*

**Phase 8.5 is built (ADR-0022 accepted; this PR).** `NodeRuntimeAdapter` (language `"typescript"`, in `packages/agent/src/grackle/node_runtime/`) drives Node over the V8 Inspector (CDP) on `127.0.0.1`, emits the same `TraceEvent` schema resolved to TS static-graph node IDs, and reuses the whole Phase 6–8 transport/UI — no wire change (`KNOWN_MESSAGE_TYPES` unchanged, `check-parity` no-op), no new deps (minimal CDP client over the existing `websockets`). **Hybrid two-channel mechanism:** `trace()` = CPU **sampling** profiler → `profile_reconstruct.reconstruct` (stack-diff over samples by V8 tree-node identity → faithful `call`/`return` with real `frame_depth` + `ts_ns`) → `--connect` replay / `-o`; `trace_streaming()` = **precise-coverage** polling → `coverage_poll` → one coarse live `call` per active function per poll (`frame_depth:0`, `metadata.live:true`, exact per-poll delta in `metadata.count`) → `--stream`. (Rendered live heat is *activity-coarse* — existing consumers count events, not `metadata.count`; magnitude-faithful heat + `grackle diff` input come from the sampling channel. Weighting consumers by `metadata.count` is a noted fast-follow.) Node-ID resolution via **type-stripping** (Node ≥ 22.6) → `NodeResolver.resolve_frame(url, line+1, functionName)` with line→name→file fallbacks; pseudo-frames/`node:`/out-of-root filtered. Lifecycle: spawn `node --inspect-brk=127.0.0.1:0 [--experimental-strip-types] bootstrap.mjs <abs script>`, parse the stderr listening URL, start profiler, `Runtime.runIfWaitingForDebugger`, race a `\x00GRACKLE_DONE` stderr sentinel vs process-exit vs timeout, then `Profiler.stop`/final coverage poll → terminate. Capability-gated on `shutil.which("node")` + version ≥ 22.6 (clean `ClickException`, never a crash); CLI `trace` now dispatches by extension/`--language` → `registry.get_runtime(lang)`. **Gotcha discovered + handled:** `Debugger.enable` closes the inspector when the script finishes, so the live path uses Profiler-only and reads offset→line maps from disk (type-stripping preserves line boundaries). Validated end-to-end against `fixtures/tiny-node-app` (Node-gated e2e runs in CI's Node-22 job). Out of scope → Phase 9: sourcemaps/compiled-`.js`, `.tsx` loaders, worker threads, Python↔Node cross-process correlation. Full plan in the memory file `project_phase_8_progress.md` + ADR-0022.

**Post-build `/code-review` (2026-05-31) logged 15 findings on PR #35; the 13 actionable ones are now FIXED in this PR** (2 were REFUTED). Highlights: coverage `--stream` exception event now stamped with a fresh `time.monotonic_ns()` (was `ts_ns=0`, mis-sorting to the front and inflating `buildCallTree` `totalNs`); `bootstrap.mjs` registers `uncaughtException`/`unhandledRejection` handlers so async errors after the top-level `await import()` resolves still surface as an exception event; the module-frame guard now treats *any* empty `functionName` as a module frame (dropped the `line<=1` heuristic that mis-resolved top-level frames sharing a line with a function decl); V8 coverage int fields are coerced safely (no more session-aborting `TypeError`); extension-less/`.pyw` scripts route back to Python while `.tsx`/`.jsx` give a clean "JSX unsupported, Phase 9" error. Plus: incremental UTF-8 stderr decode, CDP default per-command timeout, loop-safe `asyncio.run`, `(path,line)` collisions decline instead of last-write-wins, clearer timeout-vs-exit messages, and `--max-events`/synchronous-coverage caveats documented. Full gate green (750 pytest incl. Node e2e). The **refactor pass** (`node_runtime/` drift from its `python_runtime/` mirror — shared resolver base / `TraceEvent` factory / dead CDP-listener code / adapter-declared extensions) is deferred to **8.6**. Full ranked list + refactor plan in `project_phase_8_progress.md`.

`PHASE_0_SUMMARY.md` through `PHASE_7_SUMMARY.md` at the repo root are the per-phase "what shipped + acceptance grid" reference cards. `PROJECT_ACCEPTANCE.md` at the repo root contains the whole-product definition-of-done grid.
