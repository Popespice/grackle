...
**Phase 8 (analysis platform) is in progress.** Chunk numbering follows the source plan (`~/.claude/plans/outline-phase-8-analysis-platform.md`), anchored on ADRs 0018–0022. **Shipped:** **8.2** — call-tree reconstruction + flame graph (PR #31, ADR-0019); and the **tee sink** (`--stream + --output`, PR #30) — a down-payment on 8.3's `MultiSink` (ADR-0020), pulled forward and shipped standalone. (Its commit/PR/tag are labeled "8.1"; that informal label is **retired** here because it collides with the plan's actual 8.1, the aggregation engine.) **Remaining:** **8.3** server-side aggregation engine **+** trace persistence + session library (ADR-0018 + ADR-0020 — combined; the aggregation engine was moved here from the old 8.1, and the `MultiSink` tee already shipped via PR #30). Covers prefix-sum cumulative heat + coverage over seek windows, sparse index, agent-side hub/cycle into `graph.metadata`, plus SQLite session store, `serve --store`, `SessionLibraryPanel`. Load-bearing for 8.4. Then **8.4** differential analysis (ADR-0021); **8.5** polyglot Node/V8 runtime (ADR-0022); **8.6** tech-debt sweep; **8.H** ship (v0.8.0, PHASE_8_SUMMARY, PROJECT_ACCEPTANCE §D, tag v0.8.0-phase-8). The plan's old standalone **8.1 is retired** (its tee-sink slice shipped as PR #30; its aggregation engine moved into 8.3). Net: **4 chunks remain** (8.4, 8.5, 8.6, 8.H) once 8.3 lands.
