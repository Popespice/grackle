...
- **POSIX path discipline.** All path-bearing fields that cross the wire or persist (node IDs, annotation keys, manifest entries) must be POSIX-relative strings — `services/auth.py`, never `services\auth.py`. Use `grackle.paths.to_posix(p, root)`; do not call `.relative_to()` directly outside `paths.py`. A single missed call site silently diverges IDs between macOS and Windows. The `ruff PTH` ruleset guards the general case; `packages/agent/tests/test_path_discipline.py` (campaign T3-4, `docs/test-campaigns/phase-12.md`) AST-scans the whole package and enforces an exact two-site allow-list — `cli.py` and `python_runtime/node_resolution.py`, both legitimate containment predicates (they use only `relative_to`'s `ValueError` side effect, never the returned path) — failing loudly on both a new unauthorized call site and a stale allow-list entry.
