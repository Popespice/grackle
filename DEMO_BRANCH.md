...
`sys.monitoring` data. `values`, `node`, `go`, and `rust` also replay real golden traces (see below).
`poly` and the synthetic size-tier presets render static-only graphs. `watch` runs a canned graph-diff
simulation instead of a trace replay (see "What's mocked").
...
`SessionStore` seeded from the golden-trace fixtures, not a canned response.
